...
function createArtworkElement(src, index) {
    ...
    // link to generate-text.html and pass the corresponding text file as query parameter
    const oneTextFile = arrayOfText[index]; // link image to its corresponding poem
    artworkDiv.href = `./generate-text.html?text=${encodeURIComponent(oneTextFile)}`;
